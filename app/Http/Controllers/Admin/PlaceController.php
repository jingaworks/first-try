...
class PlaceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Place::where('niv', 3)->orderBy('denloc')->with(['region'])->select(sprintf('%s.*', (new Place)->table));
    ...
    public function show(Place $place)
    {
        ...
        $place->load('region', 'placeProfiles', 'placeAddresses', 'showOnPlaceFeaturedProducts');
