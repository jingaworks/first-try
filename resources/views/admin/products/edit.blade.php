...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subcategory') ? 'is-invalid' : '' }}" name="subcategory_id" id="subcategory_id">
                    @foreach($subcategories as $id => $subcategory)
                        <option value="{{ $id }}" {{ (old('subcategory_id') ? old('subcategory_id') : $product->subcategory->id ?? '') == $id ? 'selected' : '' }}>{{ $subcategory }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="address_id">{{ trans('cruds.product.fields.address') }}</label>
                <select class="form-control select2 {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address_id" id="address_id">
                    @foreach($addresses as $id => $address)
                        <option value="{{ $id }}" {{ (old('address_id') ? old('address_id') : $product->address->id ?? '') == $id ? 'selected' : '' }}>{{ $address }}</option>
                    @endforeach
                </select>
