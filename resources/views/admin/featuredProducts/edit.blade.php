        ...
        <form method="POST" action="{{ route("admin.featured-products.update", [$featuredProduct->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $featuredProduct->active || old('active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $product)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $featuredProduct->product->id ?? '') == $id ? 'selected' : '' }}>{{ $product }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('profile') ? 'is-invalid' : '' }}" name="profile_id" id="profile_id">
                    @foreach($profiles as $id => $profile)
                        <option value="{{ $id }}" {{ (old('profile_id') ? old('profile_id') : $featuredProduct->profile->id ?? '') == $id ? 'selected' : '' }}>{{ $profile }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('show_on_region') ? 'is-invalid' : '' }}" name="show_on_region_id" id="show_on_region_id">
                    @foreach($show_on_regions as $id => $show_on_region)
                        <option value="{{ $id }}" {{ (old('show_on_region_id') ? old('show_on_region_id') : $featuredProduct->show_on_region->id ?? '') == $id ? 'selected' : '' }}>{{ $show_on_region }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('show_on_place') ? 'is-invalid' : '' }}" name="show_on_place_id" id="show_on_place_id">
                    @foreach($show_on_places as $id => $show_on_place)
                        <option value="{{ $id }}" {{ (old('show_on_place_id') ? old('show_on_place_id') : $featuredProduct->show_on_place->id ?? '') == $id ? 'selected' : '' }}>{{ $show_on_place }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('show_on_category') ? 'is-invalid' : '' }}" name="show_on_category_id" id="show_on_category_id">
                    @foreach($show_on_categories as $id => $show_on_category)
                        <option value="{{ $id }}" {{ (old('show_on_category_id') ? old('show_on_category_id') : $featuredProduct->show_on_category->id ?? '') == $id ? 'selected' : '' }}>{{ $show_on_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('show_on_subcategory') ? 'is-invalid' : '' }}" name="show_on_subcategory_id" id="show_on_subcategory_id">
                    @foreach($show_on_subcategories as $id => $show_on_subcategory)
                        <option value="{{ $id }}" {{ (old('show_on_subcategory_id') ? old('show_on_subcategory_id') : $featuredProduct->show_on_subcategory->id ?? '') == $id ? 'selected' : '' }}>{{ $show_on_subcategory }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('views') ? 'is-invalid' : '' }}" type="number" name="views" id="views" value="{{ old('views', $featuredProduct->views) }}" step="1">
                @if($errors->has('views'))
                    <div class="invalid-feedback">
                        {{ $errors->first('views') }}
                    </div>
